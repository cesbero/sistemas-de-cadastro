...
AS
  ...
  V_VL_ESTOQUE_INICIAL          NUMBER(22,4) := 0;
    ...
    CURSOR CUR_MERCADORIA IS SELECT '535' AS FILIAL, MERC_CODIGO
               ...
               WHERE MERC_CODIGO IN('R00000264228','R00001099264','R00000303812','R00000500210')
...
BEGIN
  ...
  LOOP
      ...
      LOOP
        BEGIN
          ...
          BEGIN
            ...
            V_VL_BASE_ACUMULADO := V_VL_BASE_ACUMULADO + NVL(NF_ENTRADA.VL_BASE_ST,0); --V_VL_BC_ST_UNIT;
              ...
              BEGIN
                ...
                LOOP
                  BEGIN
                    ...
                    FOR NF_ENTRADA_CD IN (select * from table(USRASSISTENTEFISCAL.FN_EXCEDENTE_ENT('607', NF_SAIDA.DATA,NF_SAIDA.MERC_CODIGO)) )
                    LOOP
                      BEGIN   
                        V_BUSCA_ENTRADA := 1;
                        
                        -- CALCULANDO IMPOSTO UNITÁRIO (BASE ICMS E ST, VALOR ICMS E ST)                           
                        
                        
                            V_VL_BC_ICMS_CD    := NF_ENTRADA_CD.VL_BASE_ICMS;
                          V_VL_ICMS_CD     := NF_ENTRADA_CD.VL_ICMS;                                                                              
                          V_VL_BC_ST_CD      := NF_ENTRADA_CD.VL_BASE_ST;     

                          IF NVL(NF_ENTRADA_CD.ESTOQUE,0) <> 0 THEN V_VL_BC_ST_UNIT := NF_ENTRADA_CD.VL_BASE_ST/NF_ENTRADA_CD.ESTOQUE; ELSE V_VL_BC_ST_UNIT := 0;  END IF;                          
                          
                          V_VL_ST_CD       := NF_ENTRADA_CD.VL_ST;
                          
                          V_VL_BASE_ACUMULADO  := V_VL_BASE_ACUMULADO + NF_ENTRADA_CD.VL_BASE_ST;
                        ...
                        THEN
                          ...
                          /*
                          -- CALCULANDO IMPOSTO UNITÁRIO (BASE ICMS E ST, VALOR ICMS E ST)                           
                          V_VL_BC_ICMS_CD    := V_VL_SALDO_REF_ESTOQUE_CD * (NF_ENTRADA_CD.VL_BASE_ICMS / NF_ENTRADA_CD.ESTOQUE);
                          V_VL_ICMS_CD     := V_VL_BC_ICMS_CD * (NF_ENTRADA_CD.ALIQ_ICMS/100);                                        
                          
                          V_VL_BC_ST_UNIT    := NF_ENTRADA_CD.VL_BASE_ST / NF_ENTRADA_CD.ESTOQUE;                         
                          V_VL_BC_ST_CD      :=  V_VL_SALDO_REF_ESTOQUE_CD * V_VL_BC_ST_UNIT;                         
                          V_VL_ST_CD       := (V_VL_BC_ST_CD * (NF_ENTRADA_CD.ALIQ_ICMS/100)) - V_VL_ICMS_CD;                                                   
                            
                          V_VL_BASE_ACUMULADO  := V_VL_BASE_ACUMULADO + V_VL_BC_ST_CD;
                          */
                          
                          -- ATUALIZANDO ESTOQUE
                          V_VL_QTD_ACUMULADO      := V_VL_QTD_ACUMULADO    + V_VL_SALDO_REF_ESTOQUE_CD;
                        ...
                        ELSE
                          BEGIN
                           ...
                           /*
                            -- CALCULANDO IMPOSTO UNITÁRIO (BASE ICMS E ST, VALOR ICMS E ST)                           
                          V_VL_BC_ICMS_CD    := NF_ENTRADA_CD.ESTOQUE * (NF_ENTRADA_CD.VL_BASE_ICMS / NF_ENTRADA_CD.ESTOQUE);
                          V_VL_ICMS_CD     := V_VL_BC_ICMS_CD * (NF_ENTRADA_CD.ALIQ_ICMS/100);                                        
                          
                          V_VL_BC_ST_UNIT    := NF_ENTRADA_CD.VL_BASE_ST / NF_ENTRADA_CD.ESTOQUE;                         
                          V_VL_BC_ST_CD      :=  NF_ENTRADA_CD.ESTOQUE * V_VL_BC_ST_UNIT;                         
                          V_VL_ST_CD       := (V_VL_BC_ST_CD * (NF_ENTRADA_CD.ALIQ_ICMS/100)) - V_VL_ICMS_CD;                                                   
                          ...
                          */
                          ...
                          -- ATUALIZANDO ESTOQUE
                          V_VL_QTD_ACUMULADO      := V_VL_QTD_ACUMULADO        + NF_ENTRADA_CD.ESTOQUE;                         
                        ...
                        THEN
                          BEGIN
                            ...
                            INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT(REC_ID,DET_ID,ID_ENTRADA,filial,volume,VL_BC_ST,VL_BC_ST_UNIT,DT_FATO_GERADOR,DT_REGISTRO,FAZ_PARTE_PEDIDO) VALUES(V_REC_ID,V_DET_ID,NF_ENTRADA_CD.ID,NF_ENTRADA_CD.FILIAL,NF_ENTRADA_CD.VOLUME,NF_ENTRADA_CD.vl_base_st,V_VL_BC_ST_UNIT,NF_ENTRADA_CD.dt_fato_gerador_imposto,SYSDATE,'N');
                ...
                -- Atualizando imposto na ENTRADA da LOJA
                 UPDATE USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT
                  SET 
                    VL_BC_ST_UNIT = V_VL_BC_ST_UNIT,
                    VL_BC_ST    = V_VL_BC_ST_CD
    ...
    END LOOP; -- Fim Associando a Saída do Item a última entrada
      ...
      SUM(ESTOQUE) INTO V_VL_ESTOQUE_ACUMULADO
          FROM USRASSISTENTEFISCAL.EXCEDENTE_MG_ENTRADA_DET 
      where 
