...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      BEGIN
        V_VL_ESTOQUE_INICIAL := 0;
      END;
      END;

          -- Início Associando a Saída do Item a última entrada e calculando a Base de Cálculo Unitária (Servirá para a média)
              ...
              LOOP
                ...
                THEN
                  ...
                  WHERE
                    ...
                    --INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_LOG VALUES(V_REC_ID,V_DET_ID,V_ID_ENTRADA,'PASSO UPDATE',NF_SAIDA.FILIAL,NF_SAIDA.MERC_CODIGO,NF_SAIDA.DATA,NF_ENTRADA.ESTOQUE,NF_SAIDA.VOLUME,V_VL_SALDO_SAIDA,V_VL_ESTOQUE,NF_ENTRADA.ATUALIZAR_ESTOQUE);
                  END;
                 ELSE
                   BEGIN
                   ...
                   --INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_LOG VALUES(V_REC_ID,V_DET_ID,V_ID_ENTRADA,'PASSO NOVO',NF_SAIDA.FILIAL,NF_SAIDA.MERC_CODIGO,NF_SAIDA.DATA,NF_ENTRADA.ESTOQUE,NF_SAIDA.VOLUME,V_VL_SALDO_SAIDA,V_VL_ESTOQUE,NF_ENTRADA.ATUALIZAR_ESTOQUE);
                   END;
                END IF;

                INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT(REC_ID,DET_ID,ID_ENTRADA,filial,volume,VL_BC_ST,VL_BC_ST_UNIT,DT_FATO_GERADOR,DT_REGISTRO) VALUES(V_REC_ID,V_DET_ID,V_ID_ENTRADA,NF_ENTRADA.FILIAL,NF_ENTRADA.VOLUME,NF_ENTRADA.vl_base_st,V_VL_BC_ST_UNIT,NF_ENTRADA.dt_fato_gerador_imposto,SYSDATE);
          ...
          IF NVL(V_ENCONTROU_ENTRADA,0) = 0 THEN INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT(REC_ID,DET_ID,DT_REGISTRO) VALUES(V_REC_ID,V_DET_ID,SYSDATE); END IF;
          END;
        ...
        BEGIN
          ...
          FROM USRASSISTENTEFISCAL.EXCEDENTE_MG_ENTRADA_DET where MERC_CODIGO = MERCADORIA.MERC_CODIGO;
       ...
       LOOP
        BEGIN
          ...
          BEGIN
            ...
            WHERE
              A.REC_ID = NF_SAIDA.REC_ID AND A.DET_ID = NF_SAIDA.DET_ID;
          ...
          WHERE
            REC_ID = NF_SAIDA.REC_ID AND DET_ID = NF_SAIDA.DET_ID;
