...
AS
	...
	V_VL_BC_ICMS_UNIT                 NUMBER(19,2) := 0;
	V_VL_BC_ST_UNIT_MEDIO             NUMBER(19,2) := 0;
	V_VL_BC_ICMS_UNIT_MEDIO           NUMBER(19,2) := 0;
    V_VL_BC_ST_ACORDO_VENDIDA         NUMBER(19,2) := 0;
	V_VL_BC_ICMS_ACORDO_VENDIDA       NUMBER(19,2) := 0;
    V_VL_DIFERENCA_ST_ENTRE_BASE      NUMBER(19,2) := 0;
	V_VL_DIFERENCA_ICMS_ENTRE_BASE    NUMBER(19,2) := 0;
    V_VL_ST_CREDITO                   NUMBER(19,2) := 0;
	V_VL_ICMS_CREDITO                 NUMBER(19,2) := 0;
    ...
    V_VL_ST_MEDIO                     NUMBER(19,2) := 0;
	V_VL_ICMS_MEDIO					  NUMBER(19,2) := 0;
    V_VL_BASE_ST_ACUMULADO            NUMBER(19,2) := 0;
	V_VL_BASE_ICMS_ACUMULADO          NUMBER(19,2) := 0;
	V_VL_BASE_ST_CALCULADA            NUMBER(19,2) := 0;
	V_VL_BASE_ICMS_CALCULADA		  NUMBER(19,2) := 0;
	V_VL_BASE_ST_CALCULADA_ACUMULADO  NUMBER(19,2) := 0;
    V_VL_QTD_ACUMULADO                NUMBER(19,2) := 0;
	V_ALIQ_ST                         NUMBER(15,4) := 0;
	V_ALIQ_ICMS                       NUMBER(15,4) := 0;
	...
	V_SOMA_BC_ICMS_UNIT_CD            NUMBER(19,2) := 0;
	V_QTD_NOTA_CD                     INT;

	V_VL_ESTOQUE_INICIAL              NUMBER(22,4) := 0;
    V_VL_ESTOQUE_ACUMULADO            NUMBER(22,4) := 0;
	V_VL_ESTOQUE_INVENTARIO           NUMBER(22,4) := 0;
	...
	V_VL_BC_ICMS_JA_ASSOCIADO          NUMBER(19,2) := 0;
	V_DET_ID_JA_ASSOCIADO        	   INT 			:= 0;
	
	V_VL_BC_ST_ANTERIOR                NUMBER(19,2) := 0;
	V_VL_BC_ICMS_ANTERIOR              NUMBER(19,2) := 0;
  ...
  */
  ...
  CURSOR CUR_MERCADORIA IS
              SELECT
                DISTINCT  UF, FILIAL, EST_CODIGO, MERC_CODIGO
...
BEGIN
  ...
  */
  ...
  LOOP
	...
	V_VL_BC_ICMS_JA_ASSOCIADO			:= 0;
	V_DET_ID_JA_ASSOCIADO         		:= 0;

    V_VL_BC_ST_ANTERIOR               	:= 0;
	V_VL_BC_ICMS_ANTERIOR				:= 0;
    V_VL_QTD_ANTERIOR               	:= 0;
	V_QTD_REF 							:= 0;
	V_REF 								:= 0;
   ...
   -- Verifica se a mercadoria possui referência no último mês.
    BEGIN
      SELECT
        ESTOQUE_FINAL,BASE_ST_ACUMULADA,BASE_ICMS_ACUMULADA,VOLUME_ACUMULADO INTO  V_VL_ESTOQUE_INICIAL, V_VL_BC_ST_ANTERIOR, V_VL_BC_ICMS_ANTERIOR, V_VL_QTD_ANTERIOR
      FROM
      (
        SELECT
			S.ESTOQUE_FINAL, S.BASE_ST_ACUMULADA, S.BASE_ICMS_ACUMULADA, S.VOLUME_ACUMULADO
      ...
      EXCEPTION WHEN NO_DATA_FOUND THEN BEGIN V_VL_ESTOQUE_INICIAL := 0; V_VL_BC_ST_ANTERIOR := 0; V_VL_QTD_ANTERIOR := 0; V_VL_BC_ICMS_ANTERIOR := 0;  END;
    END;
	
	V_DATA_CORTE := NULL;
	-- INVENTARIO
	USRASSISTENTEFISCAL.FN_EXCEDENTE_INVENTARIO(MERCADORIA.FILIAL, MERCADORIA.MERC_CODIGO, to_date(DT_INICIAL, FORMATO), V_DATA_CORTE, V_VL_ESTOQUE_INVENTARIO);
	
	IF V_DATA_CORTE is null OR NVL(V_VL_ESTOQUE_INVENTARIO,0) <= 0
	THEN 
		V_DATA_CORTE := ADD_MONTHS(to_date(DT_INICIAL, FORMATO), -12); -- RETROCEDER 1 ANO PARA AS MERCADORIAS QUE NAO TEVE INVENTARIO. DIA 28-08-2023
		
		FOR NF_ENTRADA IN (select * from table(USRASSISTENTEFISCAL.FN_EXCEDENTE_ULT_ENT(P_FILIAL, to_date(DT_INICIAL, FORMATO), V_DATA_CORTE, MERCADORIA.MERC_CODIGO)) )
          LOOP
			BEGIN
				IF V_DATA_CORTE is null
				THEN 
					BEGIN
						INSERT INTO USRASSISTENTEFISCAL.GBA_EXCEDENTE_ENT_INVENTARIO (UF, FILIAL, EST_CODIGO, DT_LANCAMENTO, MERC_CODIGO, QTD_INVENTARIO, DT_PARTIDA, DT_PRIMEIRA_NOTA, DT_CRIACAO) VALUES(MERCADORIA.UF, MERCADORIA.FILIAL, MERCADORIA.EST_CODIGO, to_date(DT_INICIAL, FORMATO), MERCADORIA.MERC_CODIGO, V_VL_ESTOQUE_INVENTARIO, NF_ENTRADA.DT_FATO_GERADOR_IMPOSTO, NF_ENTRADA.DT_FATO_GERADOR_IMPOSTO, SYSDATE);
					END;
				ELSE
					BEGIN
	...
	IF V_VL_ESTOQUE_INICIAL = 0
	THEN
		V_VL_ESTOQUE_INICIAL := NVL(V_VL_ESTOQUE_INVENTARIO,0);
	END IF;

	--calcular a média a cada mês. Alterado dia 12/06/2023, após reunião com Karina e Lucas Barbosa.
    V_VL_BASE_ST_ACUMULADO 		:= 0;
	V_VL_BASE_ICMS_ACUMULADO	:= 0;
      ...
      LOOP
        BEGIN
          ...
          BEGIN
				...
				IF NVL(NF_ENTRADA.VOLUME,0)  <> 0 THEN V_VL_BC_ICMS_UNIT := NF_ENTRADA.VL_BASE_ICMS / NF_ENTRADA.VOLUME;  ELSE V_VL_BC_ICMS_UNIT := 0;   	END IF;
				...
				THEN
				  BEGIN
					  ...
					  INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT(REC_ID,DET_ID,ID_ENTRADA,filial,MERC_CODIGO,volume,ESTOQUE,VL_BC_ST,vl_bc_icms,VL_BC_ST_UNIT,VL_BC_ICMS_UNIT,DT_FATO_GERADOR,DT_REGISTRO,FAZ_PARTE_PEDIDO,VL_BC_ST_UNIT_MEDIO,VL_BC_ICMS_UNIT_MEDIO,vl_base_st_acumulada,vl_base_icms_acumulada, QTD_ACUMULADA,SALDO_SAIDA,ID_COUNT,ALIQ_STF) VALUES(V_REC_ID,V_DET_ID,NF_ENTRADA.ID,NF_ENTRADA.FILIAL,NF_SAIDA.MERC_CODIGO,NF_ENTRADA.VOLUME,V_VL_ESTOQUE,V_VL_BC_ST_JA_ASSOCIADO,V_VL_BC_ICMS_JA_ASSOCIADO,V_VL_BC_ST_UNIT,V_VL_BC_ICMS_UNIT,NF_ENTRADA.dt_fato_gerador_imposto,SYSDATE,'S',V_VL_BC_ST_UNIT_MEDIO,V_VL_BC_ICMS_UNIT_MEDIO,V_VL_BASE_ST_ACUMULADO,V_VL_BASE_ICMS_ACUMULADO,V_VL_QTD_ACUMULADO,V_VL_SALDO_SAIDA,V_REF,NF_ENTRADA.ALIQ_STF);
			  ...
			  --INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_LOG VALUES(V_REC_ID,V_DET_ID,NULL,V_REF_ID_ENTRADA,null,NF_SAIDA.FILIAL,NF_SAIDA.MERC_CODIGO,NF_ENTRADA.cfop_codigo,NF_SAIDA.DATA,NF_ENTRADA.VOLUME,NF_ENTRADA.VL_BASE_ST,V_VL_QTD_ACUMULADO,V_VL_BASE_ACUMULADO,NF_ENTRADA.ATUALIZAR_ESTOQUE,V_NOTA_JA_ASSOCIADA);
				...
				IF NF_ENTRADA.cfop_codigo = '1.409' OR NF_ENTRADA.cfop_codigo = '2.409' --PARA NOTAS DE TRANSFERENCIA, BUSCAR ENTRADA DO CD. TRAZER OS VALORES DE IMPOSTO E ATUALIZAR NA ENTRADA DA LOJA
				THEN
				  ...
				  BEGIN
					...
					BEGIN
						...
						LOOP
						  BEGIN
							...
							IF NVL(NF_ENTRADA_CD.VOLUME,0) <> 0 THEN V_VL_BC_ICMS_UNIT 	:= NF_ENTRADA_CD.VL_BASE_ICMS/NF_ENTRADA_CD.VOLUME; ELSE V_VL_BC_ICMS_UNIT := 0;  	END IF;
							...
							V_SOMA_BC_ICMS_UNIT_CD 	:= V_SOMA_BC_ICMS_UNIT_CD 	+ NVL(NF_ENTRADA_CD.VL_BASE_ICMS,0);
										...
										BEGIN	
											UPDATE USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT
												SET id_CD_REF   = NF_ENTRADA_CD.ID,
													...
													ALIQ_STF_CD = NF_ENTRADA_CD.ALIQ_STF
											WHERE
												REC_ID      	= V_REC_ID
												AND DET_ID    	= V_DET_ID
												AND ID_ENTRADA  = V_REF_ID_ENTRADA;
											...
											UPDATE USRASSISTENTEFISCAL.EXCEDENTE_MG_ENTRADA_DET
												SET ALIQ_STF_CD = NF_ENTRADA_CD.ALIQ_STF
											WHERE
												ID = V_REF_ID_ENTRADA;
					...
					IF NVL(V_QTD_NOTA_CD,0) <> 0 THEN V_VL_BC_ICMS_UNIT := V_SOMA_BC_ICMS_UNIT_CD / V_QTD_NOTA_CD; 	ELSE V_VL_BC_ICMS_UNIT := 0;  	END IF;

					V_VL_BASE_ST_CALCULADA 		:= NVL(NF_ENTRADA.VOLUME,0) * V_VL_BC_ST_UNIT;
					V_VL_BASE_ICMS_CALCULADA 	:= NVL(NF_ENTRADA.VOLUME,0) * V_VL_BC_ICMS_UNIT;
					...
					SET
					  ...
					  vl_base_st_acumulada_cd 		= V_SOMA_BC_ST_UNIT_CD,
					  
					  VL_BC_ICMS_UNIT_CD    		= V_VL_BC_ICMS_UNIT,
					  VL_BC_ICMS_UNIT    			= (CASE WHEN VOLUME = 0 THEN 0 ELSE (V_VL_BASE_ICMS_CALCULADA/VOLUME) END),
					  VL_BC_ICMS            		= V_VL_BASE_ICMS_CALCULADA,
					  vl_base_icms_acumulada_cd 	= V_SOMA_BC_ICMS_UNIT_CD,
					...
					UPDATE USRASSISTENTEFISCAL.EXCEDENTE_MG_ENTRADA_DET
					  SET 
						...
						vl_base_ICMS   	= V_VL_BASE_ICMS_CALCULADA,
						vl_bc_icms_unit = (CASE WHEN VOLUME = 0 THEN 0 ELSE (V_VL_BASE_ICMS_CALCULADA/VOLUME) END)
...
-- fazer o controle do estoque também no CD. Para teste, fazer um estudo considerando 2 meses e buscando a nota com o menor volume.
  ...
  FOR NF_ENTRADA IN ( 
					SELECT 
						...
						DISTINCT A.REC_ID, A.ID_COUNT,A.ID_ENTRADA, A.VOLUME, 
								 A.vl_bc_st VL_BASE_ST, A.VL_BC_ST_UNIT,
								 A.vl_bc_icms VL_BASE_ICMS, A.VL_BC_ICMS_UNIT
	...
	BEGIN
		...
		V_VL_BASE_ST_ACUMULADO  	:= V_VL_BASE_ST_ACUMULADO 	+ NVL(NF_ENTRADA.VL_BASE_ST,0);
		V_VL_BASE_ICMS_ACUMULADO  	:= V_VL_BASE_ICMS_ACUMULADO + NVL(NF_ENTRADA.VL_BASE_ICMS,0);
		
		V_VL_BC_ST_UNIT_MEDIO 		:= (CASE WHEN V_VL_QTD_ACUMULADO = 0 THEN V_VL_BC_ST_UNIT_MEDIO 	ELSE V_VL_BASE_ST_ACUMULADO 	/ V_VL_QTD_ACUMULADO END );		
		V_VL_BC_ICMS_UNIT_MEDIO 	:= (CASE WHEN V_VL_QTD_ACUMULADO = 0 THEN V_VL_BC_ICMS_UNIT_MEDIO 	ELSE V_VL_BASE_ICMS_ACUMULADO 	/ V_VL_QTD_ACUMULADO END );
		
		UPDATE USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT
		  SET
			vl_base_st_acumulada   	= V_VL_BASE_ST_ACUMULADO,
			QTD_ACUMULADA       	= V_VL_QTD_ACUMULADO,
			VL_BC_ST_UNIT_MEDIO 	= V_VL_BC_ST_UNIT_MEDIO,
			
			vl_base_icms_acumulada  = V_VL_BASE_ICMS_ACUMULADO,
			VL_BC_ICMS_UNIT_MEDIO 	= V_VL_BC_ICMS_UNIT_MEDIO
		WHERE
  ...
  BEGIN
    SELECT
      VL_BC_ST_UNIT_MEDIO, VL_BC_ICMS_UNIT_MEDIO INTO V_VL_ST_MEDIO, V_VL_ICMS_MEDIO
    FROM
    (
      SELECT
        DISTINCT ID_ENTRADA,ID_COUNT, VL_BC_ST_UNIT_MEDIO, VL_BC_ICMS_UNIT_MEDIO
  ...
  LOOP
      ...
      V_VL_BC_ST_ACORDO_VENDIDA 		:= V_VL_ST_MEDIO              		*  NF_SAIDA_RECUP.VOLUME;
      V_VL_DIFERENCA_ST_ENTRE_BASE 		:= V_VL_BC_ST_ACORDO_VENDIDA  		-  NF_SAIDA_RECUP.VL_CONTABIL;
      V_VL_ST_CREDITO           		:= V_VL_DIFERENCA_ST_ENTRE_BASE 	*  (NVL(V_ALIQ_ST,0)/100);
			
	  V_VL_BC_ICMS_ACORDO_VENDIDA 		:= V_VL_ICMS_MEDIO              	*  NF_SAIDA_RECUP.VOLUME;
      V_VL_DIFERENCA_ICMS_ENTRE_BASE 	:= V_VL_BC_ICMS_ACORDO_VENDIDA  	-  NF_SAIDA_RECUP.VL_CONTABIL;
      V_VL_ICMS_CREDITO           		:= V_VL_DIFERENCA_ICMS_ENTRE_BASE  	*  (NVL(V_ALIQ_ICMS,0)/100);

      UPDATE USRASSISTENTEFISCAL.EXCEDENTE_MG_SAIDA_DET
      SET
        VL_BASE_ST_UNIT   	= V_VL_ST_MEDIO,
        VL_BASE_ST_VENDA  	= V_VL_BC_ST_ACORDO_VENDIDA,
        vl_dif_base_st    	= V_VL_DIFERENCA_ST_ENTRE_BASE,
        vl_credito_st     	= V_VL_ST_CREDITO,
		aliq_stf_entrada  	= V_ALIQ_ST,
		
		VL_BASE_ICMS_UNIT   = V_VL_ICMS_MEDIO,
        VL_BASE_ICMS_VENDA  = V_VL_BC_ICMS_ACORDO_VENDIDA,
        vl_dif_base_ICMS    = V_VL_DIFERENCA_ICMS_ENTRE_BASE,
        vl_credito_ICMS     = V_VL_ICMS_CREDITO
		--,aliq_ICMS_entrada   = V_ALIQ_ICMS
      WHERE
        REC_ID      = V_REC_ID
  ...
  FOR NF_SAIDA IN (
				  ...
				  (
					  SELECT
						FILIAL, DATA, MERC_CODIGO, SUM(VL_CREDITO_ST) TOTAL_CREDITO
					  FROM USRASSISTENTEFISCAL.EXCEDENTE_MG_SAIDA_DET
					  WHERE
						 REC_ID = V_REC_ID
					  GROUP BY
						FILIAL, DATA, MERC_CODIGO
					  HAVING SUM(VL_CREDITO_ST) < 0
  ...
  BEGIN
    SELECT
		...
		SUM(VL_BASE_ST),
		SUM(VL_BASE_ICMS)
		INTO V_VL_ESTOQUE_ACUMULADO,V_VL_QTD_ACUMULADO,V_VL_BASE_ST_CALCULADA,V_VL_BASE_ICMS_CALCULADA
    FROM
	(
		SELECT DISTINCT E.*
		FROM USRASSISTENTEFISCAL.Excedente_MG_SAIDAXENT A
  ...
  INSERT INTO USRASSISTENTEFISCAL.Excedente_MG_MERC(rec_id,filial,DT_INICIO,DT_TERMINO,merc_codigo,BASE_ST_ACUMULADA, BASE_ICMS_ACUMULADA, VOLUME_ACUMULADO,estoque_inicial,estoque_final,dt_registro) VALUES(V_REC_ID,P_FILIAL,to_date(DT_INICIAL, FORMATO),LAST_DAY(TO_DATE(DT_INICIAL, FORMATO)),MERCADORIA.MERC_CODIGO,V_VL_BASE_ST_CALCULADA,V_VL_BASE_ICMS_CALCULADA,V_VL_QTD_ACUMULADO,V_VL_ESTOQUE_INICIAL,V_VL_ESTOQUE_ACUMULADO,SYSDATE);
