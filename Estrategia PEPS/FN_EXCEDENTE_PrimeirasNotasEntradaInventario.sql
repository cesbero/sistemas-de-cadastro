...
CREATE OR REPLACE PROCEDURE USRASSISTENTEFISCAL.FN_EXCEDENTE_INVENTARIO(P_FILIAL IN VARCHAR2, P_MERCADORIA IN VARCHAR2, P_PERIODO_VENDA IN DATE, P_DATA_CORTE OUT DATE)
AS
...
BEGIN
		...
		ELSE
			BEGIN
				...
				BEGIN
					...
					FOR NF_ENTRADA IN (select * from table(USRASSISTENTEFISCAL.FN_EXCEDENTE_ULT_ENT(INVENTARIO.FILIAL, INVENTARIO.dt_transacao,INVENTARIO.MERC_CODIGO)) )
					LOOP
					BEGIN
  ...
  THEN
	BEGIN
      SELECT
        G.DT_PARTIDA INTO  V_DATA_CORTE
      ...
      EXCEPTION WHEN NO_DATA_FOUND THEN BEGIN V_DATA_CORTE := to_date(extract(year from P_PERIODO_VENDA)||'/'||extract(month from P_PERIODO_VENDA)||'/01','yyyy/mm/dd');  END;
    END;
  END IF;
