...
CREATE OR REPLACE FUNCTION USRASSISTENTEFISCAL.FN_EXCEDENTE_ULT_ENT_IVT(P_FILIAL IN VARCHAR2, P_DATA_INVENTARIO IN DATE, P_MERC_CODIGO IN VARCHAR)
        ...
        SELECT CAST (
              MULTISET (
					...
					(
						...
						FROM USRASSISTENTEFISCAL.EXCEDENTE_MG_ENT_INVETARIO E
						WHERE
						  E.FILIAL                       = P_FILIAL
						  AND E.MERC_CODIGO              = P_MERC_CODIGO
						  AND E.DT_FATO_GERADOR_IMPOSTO <= P_DATA_INVENTARIO
						  AND E.ESTOQUE 				 > 0
				...
				SELECT CAST (
					 MULTISET (
								...
								(
									...
									LEFT  JOIN USRASSISTENTEFISCAL.EXCEDENTE_MG_ENT_INVETARIO U  ON   NFE.DOF_IMPORT_NUMERO = U.DOF_IMPORT_NUMERO AND NEI.MERC_CODIGO = U.MERC_CODIGO AND NEI.IDF_NUM = U.IDF_NUM
            ...
            SELECT CAST (
						 MULTISET (
								  ...
								  (					
									...
									WHERE
									  F.FILIAL                       	= P_FILIAL
									  AND NFE.DT_FATO_GERADOR_IMPOSTO  <= P_DATA_INVENTARIO
									 ...
									 -- LIMITANDO A BUSCA À 2 ANOS. ALTERAÇÃO FEITA APÓS CONVERSA COM KARINA (16/09/2022)
										AND 1 = CASE WHEN MONTHS_BETWEEN (P_DATA_INVENTARIO, NFE.DT_FATO_GERADOR_IMPOSTO) < 24 THEN 1 ELSE 0 END
